...
class TestCreateUser(TestCase):
    ...
    def test_email_modify(self) -> None:
        ...
        self.assertEqual(response['message'], "이미 가입되어있는 EMAIL입니다.")
    ...
    def test_password_modify(self) -> None:
        ...
        response = response.json()
        self.assertEqual(response['result'], "failed")
        self.assertEqual(response['message'], "비밀번호는 8자 이상, 영문, 숫자, 특수문자가 포함되어야 합니다.")
    ...
    def test_name_modify(self) -> None:
        ...
        response = response.json()
        self.assertEqual(response['result'], 'failed')
        self.assertEqual(response['message'], "이미 가입된 닉네임입니다.")
        ...
        response = response.json()
        self.assertEqual(response['result'], 'failed')
        self.assertEqual(response['message'], "닉네임은 4자이상, 영문, 숫자, _ 외에 입력이 불가능합니다.")
