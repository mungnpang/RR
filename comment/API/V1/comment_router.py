...
from comment.services import COMMENTS_READ, COMMENTS_CREATE, COMMENT_UPDATE, DELETE_COMMENT, REPLYS_READ, COMMENTS_READ_USER
...
def comment_read_user(request: HttpRequest, user: int) -> List[Comment]:
    comments = COMMENTS_READ_USER(user)
    if len(comments) == 0:
