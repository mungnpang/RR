...
$('#origin_password').on('input', function () {
    origin_check()
})
...
function origin_check(){
    ...
    if (origin_value.length > 7){
        $(`#origin_check`).attr('value','True')
        $(`#origin_check`).text('')
        $(`#origin_check`).hide()
    } else {
        $(`#origin_check`).attr('value','False')
        $(`#origin_check`).text('비밀번호는 8자이상입력하셔야 합니다.')
        $(`#origin_check`).show()
    }
}
...
function repeat_valid_check() {
    ...
    if (input_value == input2_value) {
        ...
        if ($('#password1_check').attr('value') == 'True' & $('#password2_check').attr('value') == 'True' & $('#origin_check').attr('value') == 'True'){
            $('#submit').show()
        } 
    }
    else{
